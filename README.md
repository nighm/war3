...
**⚠️ 重要提醒：在开始任何开发工作之前，必须阅读并遵循 [USER_RULES.md](USER_RULES.md)！**

这些规则定义了项目开发的核心约束，违反将导致代码审查不通过。
...
在开始任何开发工作之前，**必须**阅读并遵循 [开发规则文档](docs/development_rules.md)！

### 核心原则
1. **预开发分析原则** - 先分析现有状态，再决定开发方式
2. **信息验证原则** - 先验证关键信息，再开始编码
3. **架构遵循原则** - 遵循现有架构，优先集成而非新建
